...
##### ***Yes, you can using the total trade value per year for imports and exports. See the vandeley table which reports the deficits for each year. ***

$$\text{Trade surplus or deficit} = \text{Imports} - \text{Exports}$$
```{r}
#full_join( x y by="common ref")
vandaley <- full_join(import_total, export_total, by="ref_year") %>%
  mutate(deficit = sum_value.x - sum_value.y) %>%
  select(ref_year, deficit)
